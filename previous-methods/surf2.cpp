...
int main()
{
	...
	//load training image
	Mat object = imread ("../bb8-pictures/test.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	if (!object.data){
    ...
    for (int j=0; j<7;j++)
    {
	    ...
	    while (escapeKey != 'q')
	    {
	        ...
	        drawMatches( object, kpObject, image, kp_image, good_matches, img_matches, Scalar::all(-1), Scalar::all(-1),
